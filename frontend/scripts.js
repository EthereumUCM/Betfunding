...
function bet(isNiceBet) {
	...
	if(pid <= numProjects && pid > 0){
		if(timestamp < endDate && !verified){
			var aux = mainContractInstance.call().checkBet(pid);
			if(aux)
